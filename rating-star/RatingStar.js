...
export default class RatingStar extends Component {
    ...
    render() {
        ...
        const { starSize, starPadding, tintColor } = this.props;
        const starButtons = [];
        for (let i = 0; i < maxStars; i++) {
            const imageSource = (i + 1) <= rating ? IconSource.ic_star_selected : IconSource.ic_star;
            starButtons.push(
                <TouchableOpacity
                    activeOpacity={0.20}
                    key={i + 1}
                ...
                >
                    <Image
                        ...
                        style={{
                            width: starSize,
                            height: starSize,
                            marginLeft: i === 0 ? 0 : starPadding === -1 ? starSize / 4 : starPadding,
                            tintColor
                        }}
                    />
                </TouchableOpacity>
