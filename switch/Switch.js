...
import {ValueUtil, Text, Colors} from '@momo-kits/core';
...
const LEFT_DEFAULT =
  SWITCH_WIDTH_DEFAULT - CIRCLE_SIZE_DEFAULT - CIRCLE_MARGIN_DEFAULT;
...
export default class Switch extends Component {
  constructor(props) {
    ...
    this.backgroundColor = animated.interpolate({
      ...
      outputRange: [
        ...
        get(props, 'activeStyle.backgroundColor', Colors.pink_05_b),
      ],
      extrapolate: 'clamp',
    });
    this.disabledBackgroundColor = animated.interpolate({
      inputRange: [0, 1],
      outputRange: [
        get(props, 'disableInActiveStyle.backgroundColor', Colors.black_03),
        get(props, 'acitveDisableStyle.backgroundColor', Colors.pink_09),
      ],
      extrapolate: 'clamp',
    });
  }
  ...
  componentDidMount() {
    if (this.getValue()) {
      Animated.timing(this.getAnimated(), {
        toValue: 1,
  ...
  shouldComponentUpdate(nextProps, nextState) {
    if (nextProps.value !== null && nextProps !== this.props) {
      ...
      if (
        ValueUtil.getBool(nextProps.value) !== this.getValue() ||
  ...
  getCircleColorAnimated = () => {
    ...
    if (
      CIRCLE_COLOR_DEFAULT === colorActive &&
      CIRCLE_COLOR_DEFAULT === colorInActive
    ) {
  ...
  onPressDebounce = debounce(this.changeValue, this.duration, {
    leading: true,
    trailing: false,
  });
          ...
          <Animated.View
            ...
            <Animated.Text
              style={[
                styles.activeText,
                {
                  opacity: this.opacityTextActive,
                  backgroundColor: this.isDisabled()
                    ? this.disabledBackgroundColor
                    : this.backgroundColor,
                },
            ...
            <Animated.Text
              style={[
                styles.inActiveText,
                {
                  opacity: this.opacityTextInactive,
                  backgroundColor: this.isDisabled()
                    ? this.disabledBackgroundColor
                    : this.backgroundColor,
                },
  ...
  renderLabel = direction => {
    const {title, position, numberOfLines, disableTitleStyle, titleStyle} =
      this.props;
...
Switch.propTypes = {
  ...
  disableInActiveStyle: PropTypes.oneOfType([
    PropTypes.array,
    PropTypes.object,
  ]),
