    ...
    onSelect = (index) => {
        ...
        if (!canExpandAll) {
            if (this.refRowItems && this.refOld !== this.refRowItems[index]) {
                if (this.refOld) { this.refOld.setStatusRow(false); }
...
ExpandCollapse.propTypes = {
    ...
    sectionSubTitleStyle: PropTypes.oneOfType([PropTypes.array, PropTypes.object]),
    containerItemStyle: PropTypes.oneOfType([PropTypes.array, PropTypes.object]),
...
class ItemData extends PureComponent {
    constructor(props) {
        ...
        this.state = { isShow: props.isShow, dataHeaderChanged: {}, dataBodyChanged: {} };
    }
    ...
    onShow = throttle(() => {
        ...
        this.setState((prev) => ({ isShow: !prev.isShow }), () => {
            ...
            Animated.timing(
                this.animationView,
                { toValue: this.state.isShow ? 1 : 0, duration: 250,useNativeDriver: true },
                Easing.bounce,
            ).start();
        });
    ...
    render() {
        const {
            renderHeaderSection, renderFooterSection, containerItemStyle, sectionStyle,
            sectionTitleStyle, sectionSubTitleStyle, renderSection, renderItem, contents, index, isHighlight,
        ...
        return (
            <View style={[styles.container, containerItemStyle, (isHighlight && isShow) && { borderColor: Colors.pink_05_b }]}>
                <View style={styles.overflow}>
                    <TouchableOpacity onPress={this.onShow}>
                        <View style={[styles.sectionView, sectionStyle]}>
                            {renderSection ? renderSection(contents, index, { deg }, dataHeaderChanged, this.onChangeDataBody) : (
                                <View>
                                    <Text.Title style={[styles.title, sectionTitleStyle]}>{contents.title}</Text.Title>
                                    {
                                        contents.subTitle && <Text.SubTitle style={[styles.subTitle, sectionSubTitleStyle]}>{contents.subTitle}</Text.SubTitle>
                                    }
                                </View>
                            )}
                            <Animated.View style={{
                    ...
                    {renderHeaderSection ? renderHeaderSection(contents, index, { deg, animated: this.animationView }, dataHeaderChanged, this.onChangeDataBody) : null}
                    {
                        isShow ? renderItem && renderItem({
                            item: contents, index, dataBody: dataBodyChanged, onChangeHeader: this.onChangeDataHeader
                        })
                            : null
                    }
                    {renderFooterSection ? renderFooterSection(contents, index, { deg, animated: this.animationView }, dataHeaderChanged, this.onChangeDataBody) : null}
                </View>
            </View>
        );
    }
