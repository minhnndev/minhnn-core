...
class StringChecker {
    static check(props) {
        try {
            ...
            StringChecker.showNotify([{ lan: 'VI', result: viResult }, { lan: 'EN', result: enResult }]);
            console.log('StringChecker \t VI');
            console.log(`StringChecker \t\t MISSING KEY:${enResult.arrayKeyMissing}`);
            console.log(`StringChecker \t\t DUPLICATE VALUE:${viResult.arrayValueDuplicate}`);
            console.log(`StringChecker \t\t UNRESOLVE KEY:${viResult.arrayUnResolve}`);
            console.log('StringChecker \n');
            console.log('StringChecker \t EN');
            console.log(`StringChecker \t\t MISSING KEY:${viResult.arrayKeyMissing}`);
            console.log(`StringChecker \t\t DUPLICATE VALUE:${enResult.arrayValueDuplicate}`);
            console.log(`StringChecker \t\t UNRESOLVE KEY:${enResult.arrayUnResolve}`);

            console.log('StringChecker DONE  \n \n \n');
        } catch (error) {
    ...
    static showAlert(lan, object) {
        if (object && object.arrayKeyMissing && object.arrayKeyMissing.length > 0) {
            console.warn(`Language missing key in ${lan} `, object.arrayKeyMissing.toString());
            return true;
        }
        return false;
    }
