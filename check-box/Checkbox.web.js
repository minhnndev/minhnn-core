...
const IC_CHECKED_DEFAULT = 'https://img.mservice.io/momo_app_v2/new_version/img/appx_icon/24_notifications_check.png';
...
export default class Checkbox extends Component {
    constructor(props) {
        super(props);
        this.state = {
            value: ValueUtil.getBool(props?.defaultValue) || ValueUtil.getBool(props?.value),
            title: props?.title || {},
            ownUpdate: false
        };
    }
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        ...
        if (typeof nextProps.value === 'boolean' && nextProps.value !== prevState.value) {
    ...
    setValue = (value) => {
        ...
        this.setState((prevState) => ({ value: value || !prevState.value, ownUpdate: true }),
            () => {
    ...
    render() {
        ...
        if (canCheckChange) {
            return (
                <TouchableOpacity delay={50} activeOpacity={0.85} onPress={this.changeValue}>
                    {view}
                </TouchableOpacity>
            );
        }
    ...
    renderCheckbox = () => {
        ...
        const { checkIcon, disabled, checkType } = this.props;
        const { value } = this.state;
        let borderStyle = styles.unCheckBorder;
        let checkStyle = styles.checkbox;
        if (isChecked) borderStyle = styles.checkedBorder;
        if (disabled) { borderStyle = isChecked ? styles.disableBorder : styles.disableCheckedBorder; checkStyle = styles.disableCheck; }
        const checkIconSource = checkIcon || IC_CHECKED_DEFAULT;

        if (checkType === 'radio') {
            return (
                <View style={[styles.radioButtonSize, styles.defaultRadioButton, value ? styles.selectedButton : {}]}>
            ...
            <View style={[styles.border, borderStyle]}>
                {value ? (
                    <Image
                        ...
                        source={ValueUtil.getImageSource(checkIconSource)}
                    />
                ) : <View />}
            </View>
        );
    ...
    renderLabel = (direction) => {
        const {
 title, disabled, position, disableTitleStyle, numberOfLines, titleStyle
} = this.props;
...
Checkbox.propTypes = {
    ...
    checkIcon: PropTypes.oneOfType([PropTypes.shape({ uri: PropTypes.string }), PropTypes.string, PropTypes.number]),
