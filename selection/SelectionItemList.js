...
export default class SelectionItemList extends Component {
    ...
    componentDidMount() {
         ...
         */
        let headerView = null;
        if (typeof ListHeaderComponent === 'function') headerView = ListHeaderComponent();
        else if (ListHeaderComponent) headerView = ListHeaderComponent;
        if (!this.init && data.length > 0) {
            const headers = [];
            if (stickyHeader) {
                data.forEach((item, index) => {
                    if (item.isHeader) headers.push(headerView ? index + 1 : index);
                });
            }
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        ...
        if (this.init && newData.length > 0 && oldData.length !== newData.length) {
    ...
    getSelectedIds = (data) => {
        ...
        const condition = compareKey && typeof compareKey === 'string' && selectedItem && dataLength > 0;
        ...
        if (condition) {
            data.forEach(((item) => {
                if (selectedItem && Array.isArray(selectedItem)) {
                    ...
                    if (item[compareKey] === selectedItem[compareKey] && !selectedIds) {
    ...
    onItemPress = (item, index) => {
        this.setItemSelectedIds(item, () => {
            ...
            if (Array.isArray(selectedIds) && multiple) {
                currentCheckedItem = selectedIds.filter((id) => item[compareKey] === id).length > 0;
            } else {
                currentCheckedItem = selectedIds === item[compareKey];
            }
            if (onItemPress && typeof onItemPress === 'function') onItemPress(currentCheckedItem ? item : {}, index, selectedIds);
        });
    }
    ...
    renderItem = ({ item, index }) => {
        const {
            itemProps, renderHeaderItem, selectionIcon, selectedItemStyle, renderKey, compareKey,
            selectedItemTitleStyle, bodyKey, leftIconKey, detailKey, rightTitleKey, onItemProps = () => { }
        } = this.props;
        const { selectedIds } = this.state;

        if (item.isHeader) {
            if (typeof renderHeaderItem === 'function') return renderHeaderItem(item);
        ...
        if (selectedIds && Array.isArray(selectedIds)) {
            selected = selectedIds.filter((id) => id === item[compareKey]).length > 0;
        } else {
        ...
        const renderItemProps = () => {
            ...
            if (typeof itemProps === 'function') return itemProps(item, selected);
        };
        return (
            <SelectionItem
                {...renderItemProps()}
                ...
                style={[itemStyle, selected ? (selectedItemStyle || styles.selectedItemStyle) : {}, this.getSeparateLine(index)]}
                titleStyle={[selected ? selectedItemTitleStyle : itemProps?.titleStyle]}
    ...
    defaultHeaderItem = (item) => {
        ...
        return (
            <View style={[styles.headerItemContainerStyle, headerItemContainerStyle]}>
                <Text.Title style={[styles.headerItemStyle, headerItemStyle]}>{item.name}</Text.Title>
            </View>
        );
    }
    ...
    renderItemSeparation = ({ leadingItem }) => {
        const {
            separateLine, separateLineStyle
        } = this.props;
    ...
    render() {
        const { selectedIds, stickyHeaderIndices, contentContainerStyle } = this.state;
        const {
            listRef, stickyHeader, compareKey, renderKey
        } = this.props;

        if ((stickyHeader && stickyHeaderIndices.length === 0) || !compareKey || !renderKey) return <View />;
...
SelectionItemList.propTypes = {
    ...
    headerItemContainerStyle: PropTypes.oneOfType([PropTypes.object, PropTypes.array]),
    selectedItemTitleStyle: PropTypes.oneOfType([PropTypes.object, PropTypes.array]),
    ...
    selectionIcon: PropTypes.oneOfType([PropTypes.object, PropTypes.number, PropTypes.string]),
