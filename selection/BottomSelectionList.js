...
import {
    IconSource, Colors, ScreenUtils, BottomPopupHeader, Spacing
} from '@momo-kits/core';
...
const styles = StyleSheet.create({
    ...
    selectedItemStyle: { backgroundColor: Colors.pale_grey_seven, borderRadius: 6 },
...
export default class BottomSelectionList extends Component {
    ...
    onClose = () => {
        const {
            requestClose, navigator, onClosePopup
        } = this.props;
        if (requestClose && typeof requestClose === 'function') requestClose(onClosePopup);
    ...
    onItemPress = (item, index, selectedIndecies) => {
        ...
        if (onItemPress && typeof onItemPress === 'function') onItemPress(item, index, selectedIndecies);
    ...
    render() {
        ...
        const {
            title, body, itemProps, selectedItemStyle, data, selectedIndex, style,
            headerStyle, buttonTitle, onButtonPress, iconClosePosition, rightButtonStyle,
            rightButtonTitle, rightTitleStyle, iconType, closeTextStyle
        } = this.props;
